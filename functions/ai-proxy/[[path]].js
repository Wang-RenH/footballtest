...
/**
 * Cloudflare Pages Function：同源 /ai-proxy/*
 * 手机不必访问 *.workers.dev（国内常被拦）
 */
export async function onRequest(context) {
  ...
  if (parts.length === 0 || parts[0] === 'health') {
    return new Response('ok', {
      status: 200,
      headers: withCors(request, { 'Content-Type': 'text/plain; charset=utf-8' }),
    })
  }
  ...
  const target = base + rest + new URL(request.url).search
